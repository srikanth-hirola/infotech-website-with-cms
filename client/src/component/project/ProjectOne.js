...
const ProjectOne = ({projectStyle, portfolio,columnGap,itemShow}) => {
  ...
  return (
    <div>
      ...
      <div>
        {/* Render the filtered data in four columns */}
     ...
     <SectionTitle 
      ...
      {filters.map(filter => (
        <button key={filter.id}
         onClick={() => handleCategoryChange(filter.label)}
        //  className={activeCategory === filter.label ? 'active' : ''}
        style={{
